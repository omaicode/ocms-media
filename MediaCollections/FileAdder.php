...
use Modules\Media\Conversions\ImageGenerators\Image as ImageGenerator;
use Modules\Media\Interfaces\HasMedia;
use Modules\Media\MediaCollections\Exceptions\DiskDoesNotExist;
use Modules\Media\MediaCollections\Exceptions\FileDoesNotExist;
use Modules\Media\MediaCollections\Exceptions\FileIsTooBig;
use Modules\Media\MediaCollections\Exceptions\FileUnacceptableForCollection;
use Modules\Media\MediaCollections\Exceptions\UnknownType;
use Modules\Media\MediaCollections\File as PendingFile;
use Modules\Media\MediaCollections\Models\Media;
use Modules\Media\ResponsiveImages\Jobs\GenerateResponsiveImagesJob;
...
class FileAdder
{
    ...
    public function toMediaCollectionFromRemote(string $collectionName = 'default', string $diskName = ''): Media
    {
        ...
        if ($storage->size($this->pathToFile) > config('media.max_file_size')) {
            throw FileIsTooBig::create($this->pathToFile, $storage->size($this->pathToFile));
        }

        $mediaClass = config('media.media_model');
        /** @var \Modules\Media\MediaCollections\Models\Media $media */
        $media = new $mediaClass();

        $media->name = $this->mediaName;

        $sanitizedFileName = ($this->fileNameSanitizer)($this->fileName);
        $fileName = app(config('media.file_namer'))->originalFileName($sanitizedFileName);
    ...
    public function attachMediaManual(Media $media)
    {
        if (! $this->subject->exists) {
            $this->subject->prepareToAttachMedia($media, $this);
    ...
    protected function processMediaItem(HasMedia $model, Media $media, self $fileAdder)
    {
        ...
        if ($this->generateResponsiveImages && (new ImageGenerator())->canConvert($media)) {
            $generateResponsiveImagesJobClass = config('media.jobs.generate_responsive_images', GenerateResponsiveImagesJob::class);

            $job = new $generateResponsiveImagesJobClass($media);

            if ($customQueue = config('media.queue_name')) {
