...
use Modules\Media\MediaCollections\Events\MediaHasBeenAdded;
...
class Filesystem
{
    ...
    protected function shouldCopyFileOnDisk(RemoteFile $file, Media $media, string $diskDriverName): bool
    {
        ...
        if (count(config('media.remote.extra_headers')) > 0) {
            return false;
        }
    ...
    public function getRemoteHeadersForFile(
        ...
        $extraHeaders = config('media.remote.extra_headers');
    ...
    {
        $mediaWithOldFileName = config('media.media_model')::find($media->id);
