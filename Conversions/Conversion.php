...
namespace Modules\Media\Conversions;
...
/** @mixin \Omaicode\Image\Manipulations */
class Conversion
{
    ...
    public function __construct(string $name)
    {
        ...
        $this->manipulations = (new Manipulations())
            ->optimize(config('media.image_optimizers'))
            ->format(Manipulations::FORMAT_JPG);

        $this->fileNamer = app(config('media.file_namer'));

        $this->loadingAttributeValue = config('media.default_loading_attribute_value');

        $this->performOnQueue = config('media.queue_conversions_by_default', true);
    }
